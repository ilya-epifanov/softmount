use std::{f64::consts::PI, fs::File, io::{BufWriter, Read, Write}, path::PathBuf, str::FromStr, sync::{Arc, Mutex}};
...
use crate::{bbox::read_bbox_data, camera::Lens, offset::{EstimatedOffset, InterpolatingOffsetMapper, LinearOffsetMapper, find_offset_at_frame, find_offset_at_frame_full_bbox, offset_inliers}, optic_flow::{DebugOptions, OpticFlowRotation}, video_source::VideoSource};
...
fn main() -> Result<(), anyhow::Error> {
    ...
    match opts.subcmd {
        SubCommand::DumpOpticFlow(dump_csv) => {
            ...
            let output_filename = &dump_csv.output.unwrap_or_else(|| { video_path.with_extension("of.csv").to_string_lossy().into_owned() });

            let cap = VideoCapture::from_file(video_path.to_string_lossy().as_ref(), opencv::videoio::CAP_ANY)?;
            let cache = Store::new(Config::new(format!("{}.cache", video_path.to_string_lossy())))?;
        
            let mut frame_rotation = OpticFlowRotation::new(VideoSource::new(cap)?, camera, Some(DebugOptions {scale: 0.25}), cache.bucket(Some("optic-flow"))?);
        ...
        SubCommand::DumpGyro(dump_csv) => {
            ...
            let output_filename = &dump_csv.output.unwrap_or_else(|| bbox_path.with_extension("att.csv").to_string_lossy().into_owned());
        ...
        SubCommand::Match(match_cmd) => {
            ...
            let frame_rotation = OpticFlowRotation::new(video_source, camera, Some(DebugOptions {scale: 0.25}), cache.bucket(Some("optic-flow"))?);
            ...
            let bbox = {
                ...
                read_bbox_data(&mut fc_blackbox::BlackboxReader::from_bytes(&bytes)?)
            };
            ...
            let offset_mapper = {
                ...
                for attempt in 0..6 {
                    ...
                    info!("Attempt {} with {} samples {} frames each", attempt + 1, samples_m1 + 1, sample_length);
                    let video_timestamps = linspace(0.3, 0.7, samples_m1 + 1).map(|offset| offset * video_length_seconds as f64).collect_vec();

                    let results: Vec<EstimatedOffset> = (video_timestamps.iter().map(|o| (o, Arc::clone(&frame_rotation))).collect_vec()).par_iter_mut().map(|(video_ts, frame_rotation)| {
                        // let center_frame = (*video_ts / video_length_seconds * video_length_frames as f64) as usize;
                        find_offset_at_frame_full_bbox(frame_rotation, &bbox, camera_angle, video_fps, **video_ts, sample_length)
                    }).collect();
            ...
            let offset_mapper = {
                ...
                loop {
                    ...
                    info!("Searching for camera angle between {:.02}deg and {:.02}deg", min_angle, max_angle);

                    for angle in linspace(min_angle, max_angle, fanout) {    
                        let offsets: Vec<_> = (video_timestamps.iter().map(|o| (o, Arc::clone(&frame_rotation))).collect_vec()).par_iter_mut().map(|(video_ts, frame_rotation)| {
                            find_offset_at_frame(frame_rotation, &bbox, angle, video_fps, **video_ts, 60, offset_mapper.bbox_ts_at_video_ts(**video_ts), 10.0)
                        }).collect();

                        let offsets = offset_inliers(&offsets, 0.5);
        
                        let avg_cost = offsets.iter().map(|o| o.cost).sum::<f64>() / offsets.len() as f64;
            ...
            let offset_mapper = {
                ...
                let results: Vec<_> = (video_timestamps.iter().map(|o| (o, Arc::clone(&frame_rotation))).collect_vec()).par_iter().map(|(video_ts, frame_rotation)| {
                    find_offset_at_frame(frame_rotation, &bbox, camera_angle, video_fps, **video_ts, 120, offset_mapper.bbox_ts_at_video_ts(**video_ts), 10.0)
                }).collect();
            ...
            
            {
                ...
                let alpha = 1.0 - (-video_frame_duration / 2.0).exp();
                let mut smoothed_attitude = SlerpSmoother::new(attitude.attitude(), alpha);
                let mut bbox_frame_start = bbox.ix_at(offset_mapper.bbox_ts_at_video_ts(0.0));
    
                let output_prefix = match_cmd.output.unwrap_or_else(|| video_path.with_extension("").to_string_lossy().into_owned() );
                ...
                for frame_ix in 0..video_length_frames {
                    let frame_ts = frame_ix as f64 * video_frame_duration;
                    let range_to_apply = {
                        let bbox_frame_end = bbox.ix_at(offset_mapper.bbox_ts_at_video_ts(frame_ts));
                    ...
                    for ix in range_to_apply {
                        attitude.apply_euler_angles(bbox.pitch[ix], bbox.yaw[ix], bbox.roll[ix], bbox.timestep);
                    }
...
impl AttitudeEstimator {
    ...
    pub fn apply_euler_angles(&mut self, roll: f64, pitch: f64, yaw: f64, scale: f64) {
        self.attitude = UnitQuaternion::identity().slerp(&UnitQuaternion::from_euler_angles(roll, pitch, yaw), scale) * self.attitude;
    }
